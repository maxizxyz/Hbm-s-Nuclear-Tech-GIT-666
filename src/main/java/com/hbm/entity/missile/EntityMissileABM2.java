...
public class EntityMissileABM2 extends EntityThrowableInterp implements IChunkLoader, IRadarDetectable, IRadarDetectableNT {
	...
	public static double baseSpeed = 1.0D;

	public EntityMissileABM2(World world) {
		super(world);
	...
	public void onUpdate() {
		...
		if(!worldObj.isRemote) {
			...
			if(this.posY > 80000 && (this.tracking == null || this.tracking.isDead)) this.setDead();
	...
	protected void updateVelocity() {
		if(ticksExisted <= 100) {
			velocity = 1.0 + (ticksExisted / 100.0) * 4.0;
		} else if(ticksExisted <= 300) {
			velocity = 5.0 + ((ticksExisted - 100) / 200.0) * 20.0;
		} else {
			double calculatedVelocity = 25.0 + ((ticksExisted - 300) / 200.0) * 25.0;
			velocity = Math.min(calculatedVelocity, 50.0);
		}
	}
	...
	protected void aimAtTarget() {
		...
		if(delta.lengthVector() < 10 && activationTimer >= 100) {
			this.setDead();
	...
	public void readEntityFromNBT(NBTTagCompound nbt) {
		...
		this.activationTimer = nbt.getInteger("activation");
	}
	...
	public void writeEntityToNBT(NBTTagCompound nbt) {
		...
		nbt.setInteger("activation", this.activationTimer);
	}
	...
	public ItemStack getMissileItemForInfo() {
		return new ItemStack(ModItems.missile_abm2);
	}
}
